...
export default function App() {
  ...
  // Minta Izin Lokasi Otomatis Saat Aplikasi Dibuka
  useEffect(() => {
    (async () => {
  ...
  const openCamera = async () => {
    ...
    if (!result.canceled) {
      ...
      await getCurrentLocation(); // Ambil lokasi langsung setelah ambil foto
    }
  };
  ...
  const openGallery = async () => {
    ...
    if (!result.canceled) {
      setImage(result.assets[0].uri);
      await getCurrentLocation(); // Ambil lokasi setelah pilih foto
    }
  };

  // ☁️ Upload ke Supabase (Fungsi Week 11)
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {/* Location Info Card (Dari Week 10) */}
        {location && (
      ...
      {/* Floating Navigation (Dari Week 9, Modifikasi fungsi Save ke Upload) */}
...
const styles = StyleSheet.create({
  ...
  // Info Card (Adaptasi Week 10 untuk Geolocation)
